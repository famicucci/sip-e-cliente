const agregarCarrito = (codigo, ptoStock, lista, filas, carrito) => {
	...
	if (!productoCarrito && cantStock > 0) {
		...
		productoCarrito = modProdCarr(productoCarrito, filaOrigen, cantStock, {
			cantVar: 1,
		});

		modCantPtoStockProdCarr(producto, ptoStock, cantidad);
...
// el origen tiene el pto de stock nuevo o el origen existente
const modProdCarr = (producto, origen, cantStock, { cantVar, cantFinal }) => {
	let nuevaCant;
	...
	// establece la nueva cantidad de acuerdo al objeto que recibe
	if (cantVar) {
		nuevaCant = producto.cantidad + cantVar;
	} else if (cantFinal) {
		nuevaCant = cantFinal;
	}
...
const modCantPtoStockProdCarr = (producto, ptoStock, cantidad) => {
	const origen = producto.origen;

	const origenModificado = origen.map((x) =>
		x.ptoStockId === ptoStock ? { ...x, cantidad: cantidad } : x
	);

	return origenModificado;
};

// devuelve el punto de stock 0 si no hay mas stock del producto
const detPtoStock = (filasStockTotal, ptoStock, codigoProducto) => {
	const cant = filasStockTotal.find(
		(x) => x.ProductoCodigo === codigoProducto
	).cantidad;
